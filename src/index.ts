...
export default class WebDAVSyncPlugin extends Plugin {
	...
	public settings: WebDAVSyncSettings
	...
	async loadSettings() {
		...
		const DEFAULT_SETTINGS: WebDAVSyncSettings = {
			account: '',
			credential: '',
			webdavServerUrl: 'https://pan.ustc.edu.cn/seafdav/',
			remoteDir: '',
			remoteCacheDir: '',
			useGitStyle: false,
	...
	/**
	 * Check if account configuration is complete
	 */
	isAccountConfigured(): boolean {
		return (
			!!this.settings.webdavServerUrl &&
			this.settings.webdavServerUrl.trim() !== '' &&
