export default {
	...
	settings: {
		...
		help: {
			name: '如何获取 WebDAV 账号和密码？',
			desc: '请查阅你的 WebDAV 服务商文档',
		},
		sections: {
			account: '账号设置',
			common: '通用设置',
		...
		cache: {
			...
			dumpName: '缓存管理',
			dumpDesc:
				'插件会在您的设备上保存远程文件夹的目录信息。当您更换设备时，这些信息会丢失，需要在首次同步前重新获取。如果您的文件数量较多，获取过程可能较慢。导出功能可将这些信息保存到远程服务器，导入功能则可将数据恢复到新设备，使您无需等待即可直接同步。',
			dump: '导出',
			restoreName: '导入缓存',
			restoreDesc:
				'从远程服务器导入之前导出的缓存数据到当前设备。这样可以避免在新设备上等待漫长的文件扫描过程，让您直接进行同步。',
			restore: '导入',
			clearName: '清除本地缓存',
			clearDesc:
				'删除当前设备上的本地缓存数据。此操作无法撤销，会导致下次同步前需要重新建立缓存。仅影响当前设备，不会影响您在远程服务器中的数据。',
			clear: '清除',
			confirm: '确认清除',
			cleared: '缓存已成功清除',
			clearModal: {
	...
	sync: {
		...
		error: {
			...
			accountNotConfigured: '尚未配置 WebDAV 账号，请先在设置中配置账号信息',
		},
